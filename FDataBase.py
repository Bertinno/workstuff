...
class FDataBase:
    ...
    def addUserinfo(self, name_surname, gender, profession, username, password):
        try:
            self.__cur.execute(
                f"UPDATE users SET name_surname='{name_surname}', gender='{gender}', username='{username}', password='{password}', profession='{profession}' WHERE username='{username}'")
            self.__db.commit()
        except sqlite3.Error as e:
